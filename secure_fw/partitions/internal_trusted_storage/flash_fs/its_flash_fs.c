                                        ...
                                        struct its_flash_fs_file_info_t *info)
{
    ...
    tfm_memcpy(info->nonce, tmp_metadata.nonce, TFM_ITS_ENC_NONCE_LENGTH);
    tfm_memcpy(info->tag, tmp_metadata.tag, TFM_ITS_AUTH_TAG_LENGTH);
#endif
...
#ifdef TFM_ITS_ENCRYPTED
    tfm_memcpy(file_meta.nonce, finfo->nonce, sizeof(finfo->nonce));
    tfm_memcpy(file_meta.tag, finfo->tag, sizeof(finfo->tag));
#endif
