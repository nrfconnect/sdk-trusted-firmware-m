 ...
 *
 * \details The nonce has to be unique for every encrypted file. A pseudorandom
 *          number is highly reccomended but not enforced. Each platform need
 *          to enforce their own strategy to differentiate the nonces.
 *
...
/**
 * \brief Set the additional authenticated data for the AEAD operation.
 *
 * \param [in] ctx      AEAD context for ITS object
 * \param [in] add      Pointer to the additional data
 * \param [in] add_size Size of the additional data in bytes
 *
 * \retval TFM_HAL_SUCCESS             The operation completed successfully
 * \retval TFM_HAL_ERROR_INVALID_INPUT Invalid argument
 */
enum tfm_hal_status_t tfm_hal_its_aead_set_add(
                                         struct tfm_hal_its_auth_crypt_ctx *ctx,
                                         uint8_t *add,
                                         size_t add_size);
